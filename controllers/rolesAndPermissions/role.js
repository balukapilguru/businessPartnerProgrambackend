...
const updateRoleWithPermissions = async (req, res) => {
  try {
    ...
    const permissionsUpdated = permissions.some(permissionData => {
      const existingPermission = existingRole.Permissions.find(permission => permission.Modules[0].name === permissionData.module);
    ...
    if (permissionsUpdated) {
      const usersWithUpdatedRole = await User.findAll({ where: { profile: roleName } });
